...
The search script lives inside this skill's own directory, not the project directory. Always invoke it by its full path — do not assume a particular working directory. This skill was installed directly into this project (not via the plugin marketplace), so `${CLAUDE_PLUGIN_ROOT}` is not set here; use the path relative to the project root instead:
...
# Step 3: supplement
python ".Codex/skills/ui-ux-pro-max/scripts/search.py" "keyboard focus modal" --domain ux
