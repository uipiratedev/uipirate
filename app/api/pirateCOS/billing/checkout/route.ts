...
export async function POST(req: NextRequest) {
  try {
    ...
    const finalSuccessUrl = successUrl || `${baseRedirect}/settings/billing?success=true`;
    const finalCancelUrl = cancelUrl || `${baseRedirect}/settings/billing?canceled=true`;

    // Developer-friendly Simulation Fallback if Stripe keys are not set
    if (!stripe) {
      console.warn("⚠️ Stripe secret key is not set. Executing a sandbox simulated checkout flow...");
      
      // Simulate database update immediately
      if (type === "subscription") {
        admin.plan = "pro";
        admin.subscriptionStatus = "active";
        admin.currentPeriodEnd = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // 30 days
      ...
      const simulatedUrl = successUrl || `${baseRedirect}/settings/billing?success=simulated-${type}`;
    ...
    let session;
    if (type === "subscription") {
      const priceId = process.env.STRIPE_PRICE_ID_PRO || "price_1OvH1mC6J4Y5eQd1Placeholder"; // Fallback placeholder
      ...
      const priceId = process.env.STRIPE_PRICE_ID_TOPUP || "price_1OvH2eC6J4Y5eQd2Placeholder"; // Fallback placeholder
    ...
    return NextResponse.json({ success: false, error: err.message || "Checkout session generation failed" }, { status: 500 });
  }
}
