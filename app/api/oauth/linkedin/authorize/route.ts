...
export async function GET(req: NextRequest) {
  ...
  if (!clientId) {
    return NextResponse.json(
      { success: false, error: "LINKEDIN_CLIENT_ID is not configured on the server." },
      { status: 500 },
    );
  }
  ...
  const redirectUri = process.env.LINKEDIN_REDIRECT_URI || `${origin}/api/oauth/linkedin/callback`;
