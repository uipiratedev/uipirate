...
import { runDashboardAudit } from "@/lib/dashboardAudit";
import { rateLimit } from "@/lib/rateLimit";
import { extractIp } from "@/lib/analytics/ip";

const MAX_RESPONSE_BYTES = 3 * 1024 * 1024; // 3MB - dashboards can be large, but this bounds worst-case abuse
const FETCH_TIMEOUT_MS = 10_000;
const USER_AGENT = "Mozilla/5.0 (compatible; UIPirateDashboardAuditor/1.0; +https://uipirate.com)";
...
export async function GET(req: NextRequest) {
  ...
  const limit = rateLimit(`dashboard-audit:${ip}`, 10, 5 * 60_000);
  ...
  try {
    ...
    const audit = runDashboardAudit(html);
