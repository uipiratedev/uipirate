...
export default async function AdminDashboardPage() {
  ...
  const recentBlogs = await Post.find()
    ...
    .limit(5)
    .select(
      "title slug published createdAt views totalViews botViews duplicateViews",
    )
    .lean();
  ...
  return (
    <div className="space-y-8 px-8 py-4">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-card border border-black/5">
        <p className="text-xs font-semibold font-jetbrains-mono text-gray-400 uppercase tracking-widest mb-4">
          View Breakdown
        </p>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            {
              ...
              value: uniqueViewsAll,
              color: "#7C3AED",
              note: "Deduplicated, 24h window",
            },
            {
              label: "Repeat Visits",
              value: duplicateViewsAll,
              color: "#0EA5E9",
              note: "Same IP, within 24h",
            },
            {
              label: "Bot / Crawler",
              value: botViewsAll,
              color: "#94A3B8",
              note: "Googlebot, Bingbot, etc.",
            },
            {
              label: "Raw Total",
              value: totalViewsAll,
              color: "#FF5B04",
              note: "All hits combined",
            },
          ].map(({ label, value, color, note }) => (
            <div key={label} className="flex flex-col gap-1">
              <p
                className="text-2xl font-bold font-geist tracking-tight"
                style={{ color }}
              >
                {value.toLocaleString()}
              </p>
              <p className="text-xs font-medium text-gray-700 font-geist">
                {label}
              </p>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-card border border-black/5">
        <h2 className="text-sm font-semibold font-geist text-gray-900 mb-4 uppercase tracking-wider">
          Quick Actions
        </h2>
        <div className="flex flex-wrap gap-3">
          <Link href={getHref("/posts/create")}>
            <Button
              className="font-geist font-medium text-white text-sm h-10 px-5 rounded-xl gap-2"
              style={{ background: "#FF5B04" }}
            >
              <IconCreate /> New Post
            </Button>
          </Link>
          <Link href={getHref("/posts")}>
            <Button
              className="font-geist font-medium text-sm h-10 px-5 rounded-xl gap-2 bg-black/5 text-gray-700"
              variant="flat"
            >
      ...
      <div className="bg-white rounded-2xl p-6 shadow-card border border-black/5">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link
            className="text-xs font-medium font-geist flex items-center gap-1 transition-colors"
            href={getHref("/posts")}
            style={{ color: "#FF5B04" }}
          >
            View all →
