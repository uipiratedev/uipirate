...
const ENGINE_META: Record<Engine, { label: string; icon: string; iconClass: string }> = {
...
/* ── KeyModal ── */
const PROVIDER_INFO: Record<KeyProvider, {
  ...
  openai: {
    label: "OpenAI", icon: "●", iconClass: "text-emerald-500",
  ...
  gemini: {
    label: "Google Gemini", icon: "✦", iconClass: "text-blue-500",
    bgClass: "bg-blue-50", borderClass: "border-blue-100", textClass: "text-blue-700",
    placeholder: "AIza...",
  ...
  mistral: {
    label: "Mistral AI", icon: "◆", iconClass: "text-violet-600",
    bgClass: "bg-violet-50", borderClass: "border-violet-100", textClass: "text-violet-700",
...
function KeyModal({ open, provider, onClose, onSave }: KeyModalProps) {
  ...
  useEffect(() => {
    if (open) { setKeyValue(""); setShowKey(false); setSaved(false); setError(null); }
  }, [open, provider]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm"
      onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl border border-black/5 w-full max-w-md mx-4 p-6"
        onClick={e => e.stopPropagation()}>

        {/* Header */}
        <div className="flex items-start justify-between mb-5">
          <div>
            <p className="text-xs font-jetbrains-mono uppercase tracking-widest mb-0.5" style={{ color: "#FF5B04" }}>API Key</p>
            <h2 className="text-lg font-bold font-geist text-gray-900">
              Update {info.label} Key
            </h2>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700 transition-colors mt-1 flex items-center justify-center"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>

        {/* Provider identity strip */}
        <div className={`flex items-start gap-3 px-4 py-3 rounded-xl mb-5 ${info.bgClass} border ${info.borderClass}`}>
          <span className={`text-xl leading-none mt-0.5 flex-shrink-0 ${info.iconClass}`}>{info.icon}</span>
          <div className="min-w-0">
            <p className={`text-xs font-bold font-geist mb-0.5 ${info.textClass}`}>{info.label}</p>
            <p className="text-xs font-geist text-gray-500 leading-relaxed">{info.description}</p>
        ...
        {/* Key input */}
        <p className="text-xs font-geist text-gray-500 mb-2">{info.label} API Key</p>
        <div className="relative mb-2">
          <input type={showKey ? "text" : "password"} value={keyValue} autoFocus
            onChange={e => setKeyValue(e.target.value)} placeholder={info.placeholder}
        ...
        <p className="text-[11px] font-geist text-gray-400 mb-5 flex items-center gap-1.5">
          ...
          Encrypted with AES-256-GCM — stored in your database, never exposed to the browser.
        </p>

        {/* Actions */}
        {error && <p className="text-xs text-red-500 font-geist mb-3">{error}</p>}
        <div className="flex gap-3">
          ...
          <button onClick={handleSave} disabled={!keyValue.trim() || saving}
...
export default function AISettingsPage() {
  ...
  const [keyModal, setKeyModal]           = useState<{ open: boolean; provider: KeyProvider }>({ open: false, provider: "openai" });

  const isDefaultsDirty = defaultEngine !== initialEngine || defaultModel !== initialModel;
  ...
  useEffect(() => {
    fetch("/api/admin/ai-config").then(r => r.json()).then(d => {
      if (d.success) {
        ...
        setDefaultEngine(eng); setInitialEngine(eng);
        setDefaultModel(mdl);  setInitialModel(mdl);
      }
    }).catch(() => {});
  ...
  const openKeyModal  = (provider: KeyProvider) => setKeyModal({ open: true, provider });
  ...
  const saveKey = async (provider: KeyProvider, key: string): Promise<void> => {
    ...
    const res  = await fetch("/api/admin/ai-config", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  const saveDefaults = async () => {
    setDefaultsSaving(true); setDefaultsError(null);
    try {
      const res  = await fetch("/api/admin/ai-config", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ defaultEngine, defaultModel }) });
      const data = await res.json();
      if (!data.success) throw new Error(data.error || "Save failed");
  ...
  return (
    <div className="space-y-8 px-8 py-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <SettingCard icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#FF5B04" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>}
        ...
        <ProviderKeyCard
          ...
          title="OpenAI" badgeColor="emerald" source={openaiSource}
          description="GPT-4o, GPT-5 and newer models."
          sourceColors={{ bg: "bg-emerald-50", border: "border-emerald-100", dot: "bg-emerald-400", text: "text-emerald-700" }}
          onManageKey={() => openKeyModal("openai")} />

        {/* Gemini */}
        <ProviderKeyCard
          icon={<span className="text-blue-500 text-lg font-bold">✦</span>}
          title="Google Gemini" badgeColor="blue" source={geminiSource}
          description="Gemini 1.5 Flash, Pro and 2.0 models."
          sourceColors={{ bg: "bg-blue-50", border: "border-blue-100", dot: "bg-blue-400", text: "text-blue-700" }}
          onManageKey={() => openKeyModal("gemini")} />

        {/* Mistral */}
        <ProviderKeyCard
          icon={<span className="text-violet-600 text-lg font-bold">◆</span>}
          title="Mistral AI" badgeColor="violet" source={mistralSource}
          description="Mistral Large, Small, Nemo and Codestral models."
          sourceColors={{ bg: "bg-violet-50", border: "border-violet-100", dot: "bg-violet-400", text: "text-violet-700" }}
      ...
      <div className="bg-white rounded-2xl shadow-card border border-black/5 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-black/5">
          <div>
            <p className="text-xs font-jetbrains-mono uppercase tracking-widest text-gray-400">Active Defaults</p>
            <p className="text-sm font-geist text-gray-500 mt-0.5">Engine and model pre-selected in the writing assistant</p>
          </div>
        ...
        {!editMode ? (
          ...
          <div className="px-6 py-5 flex flex-wrap items-center gap-8">
            <div>
              <p className="text-[10px] font-jetbrains-mono uppercase tracking-widest text-gray-400 mb-1.5">Engine</p>
              <div className="flex items-center gap-2">
                <span className={`text-base ${engMeta.iconClass}`}>{engMeta.icon}</span>
                <span className="text-sm font-bold font-geist" style={{ color: "#FF5B04" }}>{engMeta.label}</span>
              </div>
            </div>
            <div className="w-px h-8 bg-black/5 hidden sm:block" />
            <div>
              <p className="text-[10px] font-jetbrains-mono uppercase tracking-widest text-gray-400 mb-1.5">Model</p>
              <span className="text-sm font-bold font-geist text-gray-800">
                {MODEL_LABELS[defaultModel] ?? defaultModel}
              </span>
            </div>
            <div className="ml-auto hidden sm:block">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-orange-50 border border-[#FF5B04]/20 text-xs font-semibold font-geist" style={{ color: "#FF5B04" }}>
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Engine selector */}
            <div>
              <p className="text-xs font-jetbrains-mono uppercase tracking-widest text-gray-400 mb-3">Default Engine</p>
              <div className="flex flex-wrap bg-black/[0.04] p-1 rounded-xl gap-1">
                {(["openai", "gemini", "mistral", "puter"] as Engine[]).map(eng => (
                  <button key={eng} onClick={() => {
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-semibold font-geist transition-all ${
                      defaultEngine === eng ? "bg-white text-gray-900 shadow-sm border border-black/5" : "text-gray-500 hover:text-gray-900"
                    }`}>
                    <span className={ENGINE_META[eng].iconClass}>{ENGINE_META[eng].icon}</span>
            ...
            {/* Model selector */}
            <div>
              <p className="text-xs font-jetbrains-mono uppercase tracking-widest text-gray-400 mb-3">Default Model</p>
            ...
            <div className="md:col-span-2 flex items-center gap-3 pt-2 border-t border-black/5">
              <button onClick={saveDefaults} disabled={!isDefaultsDirty || defaultsSaving}
              ...
              {defaultsError && <p className="text-xs text-red-500 font-geist">{defaultsError}</p>}
            </div>
          </div>
        )}
      </div>
...
const ProviderKeyCard = ({ icon, title, badgeColor, source, description, sourceColors, onManageKey }: ProviderKeyCardProps) => {
  const badge = source === "env" ? ".env ✓" : source === "db" ? "Saved" : undefined;
  return (
    <SettingCard icon={icon} title={title} badge={badge} badgeColor={badgeColor} description={description}>
      {/* Key status pill */}
      {source ? (
        <div className={`flex items-center gap-2 mt-3 px-3 py-2 rounded-lg ${sourceColors.bg} border ${sourceColors.border}`}>
          <span className={`w-2 h-2 rounded-full ${sourceColors.dot} flex-shrink-0`} />
          <span className={`text-xs font-geist ${sourceColors.text}`}>
            {source === "env" ? "Key loaded from .env" : "Encrypted key saved in database"}
          </span>
        </div>
      ) : (
        <div className="flex items-center gap-2 mt-3 px-3 py-2 rounded-lg bg-gray-50 border border-black/5">
          <span className="w-2 h-2 rounded-full bg-gray-300 flex-shrink-0" />
          <span className="text-xs font-geist text-gray-400">No key configured</span>
        </div>
      )}
      {/* Manage button */}
