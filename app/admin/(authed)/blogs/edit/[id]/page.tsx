  ...
  const blockItems = [
    {
      ...
      action: () => { setOpen(false); imageUploadRef.current?.click(); },
    },
    {
      id: "image-url",
      label: "Image URL",
      ...
      action: () => { setOpen(false); onImageUrl(); },
    },
    {
      id: "video",
      label: "Embed Video",
      ...
      action: () => { setOpen(false); onVideoEmbed(); },
    },
    {
      id: "code",
      label: "Code Block",
      ...
      action: () => { setOpen(false); editor.chain().focus().toggleCodeBlock().run(); },
    },
    {
      id: "divider",
      label: "Divider",
      ...
      action: () => { setOpen(false); editor.chain().focus().setHorizontalRule().run(); },
    },
    {
      id: "quote",
      label: "Blockquote",
      ...
      action: () => { setOpen(false); editor.chain().focus().toggleBlockquote().run(); },
    },
    {
      id: "table",
      label: "Table",
    ...
    >
      ...
      {open && (
        ...
        >
          ...
          <span className="text-[10px] font-jetbrains-mono text-gray-300 pr-1">or type /</span>
        </div>
      )}
    </div>
  );
...
const SlashCommandMenu = ({
  ...
  const commands = React.useMemo(
    () => [
      { title: "Heading 1", icon: "H1", desc: "Large section heading", command: () => editor.chain().focus().toggleHeading({ level: 1 }).run() },
      { title: "Heading 2", icon: "H2", desc: "Medium section heading", command: () => editor.chain().focus().toggleHeading({ level: 2 }).run() },
      { title: "Heading 3", icon: "H3", desc: "Small section heading", command: () => editor.chain().focus().toggleHeading({ level: 3 }).run() },
      { title: "Bullet List", icon: "•", desc: "Create a simple list", command: () => editor.chain().focus().toggleBulletList().run() },
      { title: "Numbered List", icon: "1.", desc: "Create a numbered list", command: () => editor.chain().focus().toggleOrderedList().run() },
      { title: "Task List", icon: "☑", desc: "Track tasks with checkboxes", command: () => editor.chain().focus().toggleTaskList().run() },
      { title: "Quote", icon: '"', desc: "Capture a quote", command: () => editor.chain().focus().toggleBlockquote().run() },
      { title: "Code Block", icon: "</>", desc: "Add a code snippet", command: () => editor.chain().focus().toggleCodeBlock().run() },
      { title: "Divider", icon: "—", desc: "Add a horizontal rule", command: () => editor.chain().focus().setHorizontalRule().run() },
      { title: "Image Upload", icon: "🖼", desc: "Upload an image from disk", command: () => { imageUploadRef.current?.click(); } },
      { title: "Image URL", icon: "🔗", desc: "Embed an image via URL", command: onImageUrl },
      { title: "Embed Video", icon: "▶", desc: "YouTube, Vimeo, Loom…", command: onVideoEmbed },
      {
        title: "Table",
        icon: "田",
  ...
  return (
    <div
      ...
      style={{ top: `${position.top + 8}px`, left: `${position.left}px`, background: "#fff", border: "1px solid rgba(0,0,0,0.08)", animation: "fadeSlideIn 0.12s ease" }}
    >
      <p className="px-4 py-1.5 text-[9px] font-semibold font-jetbrains-mono text-gray-400 uppercase tracking-widest">Blocks</p>
      {commands.map((command, index) => (
        ...
        >
          <span className="text-xs font-bold font-jetbrains-mono text-gray-400 w-7 text-center flex-shrink-0">{command.icon}</span>
          <div className="min-w-0">
            <div className="text-sm font-medium font-geist text-gray-800 leading-snug">{command.title}</div>
            <div className="text-[11px] font-geist text-gray-400 leading-snug">{command.desc}</div>
          </div>
        </button>
      ))}
    </div>
        ...
        keydown: (view, event) => {
          if (event.key === "/") {
            ...
            if (textBefore === "" || textBefore.endsWith(" ")) {
              setTimeout(() => {
                const coords = view.coordsAtPos(selection.from);
                setSlashMenuPosition({ top: coords.bottom + window.scrollY, left: coords.left + window.scrollX });
    ...
    <div className="min-h-screen" style={{ background: "#F7F7F6" }}>
      ...
      <div className="flex gap-6 p-6 items-start">
        ...
        <div className="flex-1 min-w-0 bg-white rounded-2xl shadow-sm border border-black/5 overflow-hidden">
          ...
          {bannerImage ? (
            ...
            <label className="flex items-center gap-2 px-10 pt-6 pb-2 cursor-pointer group w-fit">
              ...
              <span className="text-xs font-geist text-gray-300 group-hover:text-[#FF5B04] transition-colors">Add cover image</span>
              <input type="file" accept="image/*" className="hidden" onChange={handleBannerImageUpload} />
            </label>
          )}
          ...
          <div ref={editorRef} className="relative px-2 py-4">
            ...
            />
            <input ref={inlineImageUploadRef} type="file" accept="image/*" className="hidden" onChange={handleInlineImageUpload} />
        ...
        <div className="w-72 flex-shrink-0 space-y-4">
          ...
          {/* Quick insert card */}
          <div className="bg-white rounded-2xl border border-black/5 shadow-sm p-4">
            <p className="text-[10px] font-jetbrains-mono text-gray-400 uppercase tracking-widest mb-3">Quick Insert</p>
            <div className="space-y-1">
              ...
              <button onClick={() => setShowImageUrlModal(true)}
