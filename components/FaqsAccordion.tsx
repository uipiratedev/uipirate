...
export default function FaqsAccordion({
  ...
  const sourceItems = items ?? LANDING_FAQS;
  const effectiveLimit = limit ?? (items ? undefined : 4);
  const displayItems =
    effectiveLimit !== undefined
      ? sourceItems.slice(0, effectiveLimit)
      : sourceItems;
      ...
      <Reveal distance="sm" variant="up">
        ...
        >
          {displayItems.map((item, index) => {
            ...
            const title1 = item.title1 || item.answer;
            ...
            return (
              ...
              >
                <div className="pt-0">
                  {title1 && (
                    <p className="mb-3 text-[15px] text-gray-600 leading-relaxed">
                      {title1}
                    </p>
                  )}
                  {list.length > 0 && (
