...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({
  ...
  <div
    className={`scroll-stack-card relative w-full mx-auto md:h-[400px] max-md:h-auto p-6 sm:p-8 md:p-10 lg:p-12 rounded-3xl md:rounded-[40px] shadow-2xl box-border origin-top will-change-transform ${itemClassName}`.trim()}
    style={{
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (useWindowScroll) {
        ...
        if (
          nextCardIndex < cardsRef.current.length &&
          originalCardPositionsRef.current.length > nextCardIndex
        ) {
          const nextCardOriginalTop =
            originalCardPositionsRef.current[nextCardIndex];
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardTop = useWindowScroll
            ? (originalCardPositionsRef.current[j] ??
