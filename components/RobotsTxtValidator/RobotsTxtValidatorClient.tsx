  ...
  const handleFetch = async () => {
    ...
    try {
      const res = await fetch(
        `/api/check-ai-bots?url=${encodeURIComponent(testUrl.trim())}`,
      );
      ...
      if (data.rawRobotsTxt) {
        ...
        setFetchMsg(
          `Loaded ${data.rawRobotsTxt.split("\n").length} lines from ${data.domain}`,
        );
      } else {
  ...
  const analysis = useMemo(() => {
    ...
    lines.forEach((rawLine, index) => {
      ...
      if (lower.startsWith("user-agent:")) {
        ...
        if (
          currentUserAgent.toLowerCase() === "gptbot" &&
          (path === "/" || path === "")
        ) {
        ...
        if (
          currentUserAgent.toLowerCase() === "perplexitybot" &&
          path === "/"
        ) {
    ...
    if (!hasUserAgent) {
      issues.push({
        ...
        message:
          "Your file contains no 'User-agent:' block. Robots.txt requires at least one User-agent specification.",
      });
    }
    ...
    if (hasWildcardDisallowRoot) {
      issues.push({
        ...
        message:
          "'User-agent: * Disallow: /' blocks ALL crawlers, including Google and AI search engines, from your entire site.",
      });
    }
    ...
    if (blocksGPTBot) {
      issues.push({
        ...
        message:
          "Explicitly disallowing GPTBot prevents ChatGPT from learning about and citing your content.",
      });
    }
    ...
    if (blocksClaudeBot) {
      issues.push({
        ...
        message:
          "Anthropic's ClaudeBot is disallowed, preventing citations in Claude responses.",
      });
    }
    ...
    if (blocksPerplexity) {
      issues.push({
        ...
        message:
          "Perplexity AI is blocked from fetching live web answers from your pages.",
      });
    }
    ...
    if (!hasSitemap) {
      issues.push({
        ...
        message:
          "Consider adding 'Sitemap: https://yourdomain.com/sitemap.xml' to help crawlers discover your URLs.",
      });
    }
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] relative overflow-hidden">
      ...
      <div className="container mx-auto px-32 lg:px-20 max-md:px-4 pt-32 pb-20 relative z-10">
        ...
        >
          <div className="mb-6 flex flex-row items-center justify-center">
            <GlassBadge variant="gradient">
              FREE LINTER &amp; SYNTAX AUDIT
            </GlassBadge>
          </div>

          <h1 className="hero-header">
            robots.txt{" "}
            <span className="text-[#FF5B04]">Validator &amp; Linter</span>
          </h1>
          <p className="text-base sm:text-lg text-gray-500 max-w-3xl mx-auto text-center font-normal leading-relaxed">
            Test and validate any robots.txt syntax. Catch blocking errors,
            unknown directives, and AI engine accessibility issues in real time.
          </p>
        </motion.div>
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <div
                  className={`px-3 py-1 rounded-full text-xs font-bold ${analysis.score >= 85
              ...
              <div className="space-y-3">
                ...
                {analysis.issues.length === 0 ? (
                  <div className="p-4 rounded-xl bg-emerald-50 text-emerald-800 text-xs font-medium flex items-center gap-2">
                    ...
                    Perfect! No syntax errors or major AI blocking rules
                    detected.
                  </div>
                ) : (
                  <div className="space-y-2.5 max-h-96 overflow-y-auto pr-1">
                    {analysis.issues.map((issue, idx) => (
                      <div
                        key={idx}
                        className={`p-3.5 rounded-xl border text-xs leading-relaxed ${issue.type === "error"
                      ...
                      >
                        <div className="flex items-center gap-2 font-bold mb-0.5">
                          ...
                          {issue.line && (
                            <span className="font-mono opacity-60 font-normal">
                              Line {issue.line}
                            </span>
                          )}
                        </div>
        ...
        <section className="mt-24 pt-14 border-t border-gray-200 max-w-5xl mx-auto space-y-16">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <p className="text-xs text-gray-500 mt-2 leading-relaxed">
              Robots Exclusion Protocol syntax errors can silently de-index
              entire sections of your website or completely lock out AI search
              citation crawlers like ChatGPT and Perplexity.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
              ...
              <h3 className="text-sm font-bold text-gray-900 mb-2 font-jakarta">
                Syntax & Directive Check
              </h3>
              <p className="text-xs text-gray-500 leading-relaxed">
                Checks for unrecognized directives, missing User-Agent
                declarations, case-sensitivity flaws, and missing leading
                slashes in path rules.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
              ...
              <h3 className="text-sm font-bold text-gray-900 mb-2 font-jakarta">
                AI Crawler Block Detection
              </h3>
              <p className="text-xs text-gray-500 leading-relaxed">
                Flags whether your file blocks critical AI search engines
            ...
            <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm">
              ...
              <p className="text-xs text-gray-500 leading-relaxed">
                Ensures that all{" "}
                <code className="font-mono text-gray-800">Sitemap:</code>{" "}
                directives provide valid, absolute HTTPS URLs reachable by
                automated bots.
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                ...
                <p className="text-xs text-gray-600 leading-relaxed">
                  Writing{" "}
                  <code className="font-mono text-gray-800">Disallow: /</code>{" "}
                  under{" "}
                  <code className="font-mono text-gray-800">User-agent: *</code>
                  , which unintentionally blocks every search engine on the
                  internet from indexing your site.
                </p>
              </div>
              <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                <h4 className="text-xs font-bold text-gray-900 mb-1">
                  Are Crawl-delay directives supported by Google?
                </h4>
                <p className="text-xs text-gray-600 leading-relaxed">
                  No. Googlebot ignores the{" "}
                  <code className="font-mono text-gray-800">Crawl-delay:</code>{" "}
                  directive. Bing and Yandex still support it, but Google
                  recommends managing crawl rate in Search Console instead.
                </p>
              </div>
            </div>
          </div>
        ...
        <SuggestedTools
          category="ai-geo"
          currentToolId="robots-txt-validator"
        />
      </div>
    </div>
  );
