...
export default function UpgradePrompt({
    ...
    try {
      const res = await fetch("/api/pirateCOS/billing/checkout", {
        ...
        body: JSON.stringify({
          type,
          successUrl:
            window.location.href.split("?")[0] + `?success=simulated-${type}`,
  ...
  const isSubdomain =
    ...
    (window.location.hostname.startsWith("cos.") ||
      window.location.hostname === "cos.uipirate.com");
  const billingUrl = isSubdomain
    ? "/settings/billing"
    : "/pirateCOS/settings/billing";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        <div className="space-y-3 my-6">
          ...
          >
            <div className="space-y-0.5">
              <span className="text-[10px] font-jetbrains-mono text-[#FF5B04] uppercase tracking-wider font-bold">
                Pro Subscription
              </span>
              ...
              <p className="text-[10px] text-gray-400">
                Unlock 500 daily credits, content API, & scheduled posts.
              </p>
            </div>
          ...
          >
            <div className="space-y-0.5">
              <span className="text-[10px] font-jetbrains-mono text-gray-400 uppercase tracking-wider font-bold">
                Credit Booster
              </span>
              <h4 className="font-bold text-gray-800 text-sm">
                Buy 1,000 Credits Booster
              </h4>
              <p className="text-[10px] text-gray-400">
                Add a one-time boost of 1,000 shared pool credits.
              </p>
            </div>
          ...
          >
            <div className="space-y-0.5">
              <span className="text-[10px] font-jetbrains-mono text-green-600 uppercase tracking-wider font-bold">
                BYOK Bypass
              </span>
              <h4 className="font-bold text-gray-800 text-sm">
                Bring Your Own Keys
              ...
              <p className="text-[10px] text-gray-400">
                Connect personal API keys for unlimited free credits.
              </p>
            </div>
        ...
        {/* Errors Block */}
        {errorMsg && (
          <p className="text-xs text-red-500 font-medium text-center mb-4 leading-normal">
            {errorMsg}
          </p>
        )}
